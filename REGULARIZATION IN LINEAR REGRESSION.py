...
# Show indices of nonzero ideal coefficients
...
print("\n Predict y from Custom X")

try:
    # Prompt user for a feature value
    x_val = float(input("Enter a value for X: "))

    # Prepare the input in the same shape as training
    X_new = np.array([[x_val]])

    # Ideal (noise-free) prediction
    y_ideal_new = 4 + 3 * X_new

    # Predictions from the models trained on outliers
    y_lin_outlier   = lin_reg.predict(X_new)
    y_ridge_outlier = ridge_reg.predict(X_new)
    y_lasso_outlier = lasso_reg.predict(X_new)

    # Predictions from the models trained on clean data
    # (you re‑fitted these later in the script)
    y_lin_clean   = linear.predict(X_new)
    y_ridge_clean = ridge.predict(X_new)
    y_lasso_clean = lasso.predict(X_new)

    print(f"\nFor X = {x_val:.2f}:")
    print(f"  Ideal (no noise)        → y = {y_ideal_new[0,0]:.2f}")
    print(f"  Linear (with outliers)  → y = {y_lin_outlier[0,0]:.2f}")
    print(f"  Ridge (with outliers)   → y = {y_ridge_outlier[0,0]:.2f}")
    print(f"  Lasso (with outliers)   → y = {y_lasso_outlier[0,0]:.2f}")
    print(f"  Linear (clean data)     → y = {y_lin_clean[0,0]:.2f}")
    print(f"  Ridge (clean data)      → y = {y_ridge_clean[0,0]:.2f}")
    print(f"  Lasso (clean data)      → y = {y_lasso_clean[0,0]:.2f}")
