...
print("Breast Cancer Prediction Based on Custom Input")
...
try:
    ...
    # Convert to 2D array and scale using the same scaler
    user_array = np.array([user_input])
    user_scaled = scaler.transform(user_array)

    # Add noise if you trained on noisy data
    user_noisy = user_scaled + noise_factor * np.random.normal(loc=0.0, scale=1.0, size=user_scaled.shape)

    # Predict with KNN and SVM
    pred_knn = knn.predict(user_noisy)[0]
    pred_svm = svm.predict(user_noisy)[0]

    # Output prediction results
    print(f"KNN Prediction: {'Malignant' if pred_knn == 0 else 'Benign'}")
    print(f"SVM Prediction: {'Malignant' if pred_svm == 0 else 'Benign'}")
